...
ec_climate_search_locations("vancouver", timeframe = "daily", year = 2015)

d<-as.data.frame(ec_climate_data(c("QUEBEC/JEAN LESAGE INTL QC 26892","VANCOUVER INTL A BC 51442"), timeframe = "daily",start = "2015-01-01", end = "2015-12-31"))
d$jul<-as.numeric(format(d$date,"%j"))
d$temp<-d$mean_temp_c
...
g<-gam(temp~s(jul,by=location),data=d)
...
#plot(g,residuals=TRUE,pch=1)
	...
	points(d$lon,d$lat)
		...
		vis.gam(g,plot.type="contour",contour.col="black",ticktype="detailed",asp=1)
	points(d$lon,d$lat)
```
